...
if (window.ActiveXObject || "ActiveXObject" in window){//判断是IE浏览器
          ...
          // alert("not ie");
                 ...
                 if (Wwin>480) {
                    // nolook();
                    $('body').empty().append('<div class="PCIE"><p>请扫描以下二维码在手机端查看</p><img src="img/thisEr.png?t=3"  /></div>');
                }
          }
}
PCIE();
...
var mySwiper = new Swiper('.swiper-container',{
  ...
  loop: true,
  loopAdditionalSlides:3, // loop后要设置这里 否则滑动到书香或者某个其前面就会空着
...
var audio = $('#sound'),
    audiobtn =  $('#audiobtn'),
    isPlaying = false; // 根据isPlaying状态判断pc不自动播放 微信自动播放
function playAudio() {
...
function audioControl(){
     $('#audiobtn').click(function(){
         if ($('#audiobtn').attr('class')=='on') {//判断audiobtn的CLass名
...
// pc上不执行这条 专门为微信准备的 pc chrome不能自动播放音乐了，所以isplaying要在微信中进行判断不能再加载完成函数中判断
