...
function dialog(text) {
    $('#dialog').text(text).fadeIn(function() {
        setTimeout(function() {
...
function photosStart(obj) {
    

    var boxHeight = obj.boxHeight;
    var boxWidth = obj.boxWidth;

    var imgHeight = IsPC() ? 200 : 115;
    ...
    function disperse(){
        ...
        var spaces_w    = boxWidth / (per_line + 1);

        //center point for first thumb along the height of the window

        var spaces_h    = boxHeight / (per_col + 1);
    ...
    function setWrapperBG(image_src) {
        $im_wrapper.css({
    ...
    $thumbs.bind('click',function(){
        ...
        if(mode == 'grid'){
            ...
            var image_src   = $this.data('origin');

            $('#photos').addClass('fixed');
    ...
    $im_next.bind('click',function(){
        ...
        if($next_thumb.length > 0){
            ...
            var image_src   = $next_thumb.data('origin');
    ...
    $im_prev.bind('click',function(){
        ...
        if($prev_thumb.length>0){
            ...
            // 上一页
            var image_src = $prev_thumb.data('origin');
    ...
    // 点击遮罩层关闭
    $('#im-cover-close').bind('click', function() {
        if ($('#photos').hasClass('fixed')) {
            flg_click = true;

            $im_wrapper.removeAttr('style');
            $photos.removeClass('fixed');
