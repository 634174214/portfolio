var openmenu=function(){
      ...
      var flag='close';
       menuCl.click(function(){
                if (flag=='close') {
                            menuCl.addClass('opened');
                            menu.fadeIn(function(){
                                    $('#header').addClass('headfix');
                            });
                            flag='open';
                }else{
                          menuCl.removeClass('opened');
                          menu.fadeOut();
...
// 下拉固定
  $.fn.navfixed= function(element) {
            var navfix = function(element) {
                var top = element.offset().top,
                       mainLeft=element.offset().left;
                $(window).scroll(function() {
                          var scrolls = $(this).scrollTop();
                           // console.log(scrolls);
                          if (scrolls >= top) {
                                  element.css('paddingLeft',mainLeft);
                                  element.addClass('fixtop');
                          }else {
                                   element.removeClass('fixtop');
                                   element.css('paddingLeft',0);
                          }
                });
        };
